...
This agent is a thin wrapper over the **`plan` skill**, which is the source of truth for
the workflow. Load and follow `.claude/skills/plan/SKILL.md` step by step:

1. Scale check — confirm this is a single-PR change; otherwise escalate to the SDD pipeline.
2. Grill the plan (mandatory) via the `grill-me` skill.
